...
- `issue_comment`: created (restricted — see Security section)
...
**Restrictions:** Manual trigger requires:
- Comment is attached to a pull request (not an issue)
...
### Issue Comment Trigger Restriction

The workflow does NOT execute for arbitrary issue comments. The `if` condition enforces:
...
if: |
  ...
  (
    ...
    (
      contains(github.event.comment.body, '/open-code-review') ||
      contains(github.event.comment.body, '@open-code-review')
    ) &&
    (
...
This prevents:
- Arbitrary users from triggering reviews
- Spending the OpenRouter credential without authorization
- Running reviews on non-PR comments
...
|------|------|-------------|
| `OCR_LLM_URL` | Secret | OpenRouter endpoint: `https://openrouter.ai/api/v1/chat/completions` |
| `OCR_LLM_AUTH_TOKEN` | Secret | OpenRouter API key |
| `OCR_LLM_MODEL` | Variable | Model slug (e.g., `openai/gpt-4o`) |
| `OCR_LLM_USE_ANTHROPIC` | Variable | `false` for OpenRouter (OpenAI-compatible) |

### LLM Protocol

The workflow explicitly sets `llm_protocol: openai` for OpenRouter compatibility.
...
OpenCodeReview Triggered
        ↓
OCR Installs & Configures LLM (OpenRouter)
        ↓
